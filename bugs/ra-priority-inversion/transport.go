...
func (t *OrchestratorTransport) Send(to string, msg Message) {
	t.holdMu.Lock()
	if t.holdActive && to == t.holdTo && msg.Kind == t.holdKind && t.heldMsg == nil {
		t.heldMsg = &heldSend{to: to, msg: msg}
		t.holdMu.Unlock()
		return
	}
	t.holdMu.Unlock()
...
func (t *OrchestratorTransport) HoldNext(to string, kind MsgKind) {
	t.holdMu.Lock()
...
func (t *OrchestratorTransport) ReleaseHeld() {
	t.holdMu.Lock()
	held := t.heldMsg
	t.holdActive = false
	t.heldMsg = nil
...
func (t *OrchestratorTransport) enqueue(to string, msg Message) {
	peer, ok := t.peers[to]
	if !ok {
		return
	}
