...
func benchSetup(o *orchestrator.Orchestrator) {
	...
	transports["A"].DelayNext(MsgKVGetReply, leaseTTL*5)
	addKVNode(o, transports)
	addLeaseExpiryNodes(o, addrs, transports, activeContenders)
}
...
func TestBench_CHESS_GlobalOnly(t *testing.T) {
	...
	logBenchStart(t, "CHESS(G-only,k=1)")
	algo := &orchestrator.CHESS{Bound: 1, GlobalOnly: true}
	...
	logBenchDone(t, "CHESS(G-only,k=1)", r.Runs, r.FirstBug, r.Elapsed)
	requireFirstBug(t, "CHESS(G-only,k=1)", r.FirstBug)
	if dir := benchDir(); dir != "" {
...
func TestBench_CHESS_GL(t *testing.T) {
	runtime.GOMAXPROCS(8)
	logBenchStart(t, "CHESS(G+L,k=1)")
	algo := &orchestrator.CHESS{Bound: 1}
	opts := append([]orchestrator.ExploreOption{orchestrator.GlobalMaxRuns(benchMaxRuns)}, observers(t, "chess-gl")...)
	orch := orchestrator.New()
	r := orch.ExploreWith(t, benchSetup, algo, opts...)
	logBenchDone(t, "CHESS(G+L,k=1)", r.Runs, r.FirstBug, r.Elapsed)
	requireFirstBug(t, "CHESS(G+L,k=1)", r.FirstBug)
	if dir := benchDir(); dir != "" {
...
func TestBench_PCT_d1(t *testing.T) {
	runtime.GOMAXPROCS(8)
	logBenchStart(t, "PCT(d=1)")
	algo := &orchestrator.PCT{Depth: 1, MaxSteps: 256, Seed: 1}
	opts := append([]orchestrator.ExploreOption{orchestrator.GlobalMaxRuns(benchMaxRuns)}, observers(t, "pct-d1")...)
	orch := orchestrator.New()
	r := orch.ExploreWith(t, benchSetup, algo, opts...)
	logBenchDone(t, "PCT(d=1)", r.Runs, r.FirstBug, r.Elapsed)
	requireFirstBug(t, "PCT(d=1)", r.FirstBug)
}
