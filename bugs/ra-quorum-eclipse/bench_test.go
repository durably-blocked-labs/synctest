...
func TestBench_CHESS_GlobalOnly(t *testing.T) {
	...
	logBenchStart(t, "CHESS(G-only,k=5)")
	algo := &orchestrator.CHESS{Bound: 5, GlobalOnly: true}
	...
	logBenchDone(t, "CHESS(G-only,k=5)", r.Runs, r.FirstBug, r.Elapsed)
	requireFirstBug(t, "CHESS(G-only,k=5)", r.FirstBug)
	if dir := benchDir(); dir != "" {
...
func TestBench_CHESS_GL(t *testing.T) {
	runtime.GOMAXPROCS(8)
	logBenchStart(t, "CHESS(G+L,k=5)")
	algo := &orchestrator.CHESS{Bound: 5}
	opts := append([]orchestrator.ExploreOption{orchestrator.GlobalMaxRuns(benchMaxRuns)}, observers(t, "chess-gl")...)
	orch := orchestrator.New()
	r := orch.ExploreWith(t, benchSetup, algo, opts...)
	logBenchDone(t, "CHESS(G+L,k=5)", r.Runs, r.FirstBug, r.Elapsed)
	requireFirstBug(t, "CHESS(G+L,k=5)", r.FirstBug)
	if dir := benchDir(); dir != "" {
