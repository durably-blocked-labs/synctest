...
func TestBench_CHESS_GlobalOnly(t *testing.T) {
	...
	logBenchStart(t, "CHESS(G-only,k=3)")
	...
	logBenchDone(t, "CHESS(G-only,k=3)", r.Runs, r.FirstBug, r.Elapsed)
...
func TestBench_CHESS_GL(t *testing.T) {
	...
	orch := orchestrator.New()
	r := orch.ExploreWith(t, benchSetup, algo, opts...)
	logBenchDone(t, "CHESS(G+L,k=3)", r.Runs, r.FirstBug, r.Elapsed)
