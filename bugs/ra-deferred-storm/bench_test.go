...
func TestBench_CHESS_GlobalOnly(t *testing.T) {
	...
	if r.FirstBug == -1 {
		t.Fatal("CHESS(G-only,k=4) did not find the deferred-storm bug")
	}
	if dir := benchDir(); dir != "" {
...
func TestBench_CHESS_GL(t *testing.T) {
	...
	if r.FirstBug == -1 {
		t.Fatal("CHESS(G+L,k=4) did not find the deferred-storm bug")
	}
	if dir := benchDir(); dir != "" {
...
func TestBench_PCT_d2(t *testing.T) {
	...
	algo := &orchestrator.PCT{Depth: 2, MaxSteps: 384, Seed: 1}
	...
	if r.FirstBug == -1 {
		t.Fatal("PCT(d=2) did not find the deferred-storm bug")
	}
}
...
func TestBench_PCT_d3(t *testing.T) {
	...
	algo := &orchestrator.PCT{Depth: 3, MaxSteps: 384, Seed: 1}
	...
	if r.FirstBug == -1 {
		t.Fatal("PCT(d=3) did not find the deferred-storm bug")
	}
}

func TestBench_PCT_d4(t *testing.T) {
	runtime.GOMAXPROCS(8)
	logBenchStart(t, "PCT(d=4)")
	algo := &orchestrator.PCT{Depth: 4, MaxSteps: 384, Seed: 1}
	opts := append([]orchestrator.ExploreOption{orchestrator.GlobalMaxRuns(benchMaxRuns)}, observers(t, "pct-d4")...)
	orch := orchestrator.New()
	r := orch.ExploreWith(t, benchSetup, algo, opts...)
	logBenchDone(t, "PCT(d=4)", r.Runs, r.FirstBug, r.Elapsed)
	if r.FirstBug == -1 {
		t.Fatal("PCT(d=4) did not find the deferred-storm bug")
	}
}
...
func TestBench_Random(t *testing.T) {
	...
	if r.FirstBug == -1 {
		t.Fatal("Random did not find the deferred-storm bug")
	}
}
