...
func TestBench_CHESS_GlobalOnly(t *testing.T) {
	...
	t.Logf("CHESS(G-only,k=4): %d runs, first_bug=%d, elapsed=%s", r.Runs, r.FirstBug, r.Elapsed)
	if dir := benchDir(); dir != "" {
...
func TestBench_CHESS_GL(t *testing.T) {
	...
	orch := orchestrator.New()
	r := orch.ExploreWith(t, benchSetup, algo, opts...)
	t.Logf("CHESS(G+L,k=4): %d runs, first_bug=%d, elapsed=%s", r.Runs, r.FirstBug, r.Elapsed)
	if dir := benchDir(); dir != "" {
...
func TestBench_PCT_d4(t *testing.T) {
	runtime.GOMAXPROCS(8)
	algo := &orchestrator.PCT{Depth: 4, MaxSteps: 256, Seed: 1}
	opts := append([]orchestrator.ExploreOption{orchestrator.GlobalMaxRuns(benchMaxRuns)}, observers(t, "pct-d4")...)
	orch := orchestrator.New()
	r := orch.ExploreWith(t, benchSetup, algo, opts...)
	t.Logf("PCT(d=4): %d runs, first_bug=%d, elapsed=%s", r.Runs, r.FirstBug, r.Elapsed)
}
